...
class AccountObserver {
  ...
  private readableTxType(num: number): string {
    const readable = nem.TransactionType[num];
  ...
  private getRecipientFromTx(tx: nem.Transaction): string[] {
    ...
    const recipient = this.isTransferTx(tx) ? tx.recipientAddress : '';
...
optParse.subscribe(
  'verbose',
  (arg: string) => { return (/^-v$/).test(arg) }
);
const option = optParse.parse();
const url = option.get('url');
const addr = option.get('address');

const observer = new AccountObserver(addr, url, option);
